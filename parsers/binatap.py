...
SOURCE = 'binatap'
SOURCE_NAME = 'BinaTap.az'
BASE_URL = 'https://binatap.az'


def parse_binatap(pages=2):
    results = []
    for page in range(1, pages + 1):
        ...
        cards = (soup.select('.post') or
                 soup.select('.item') or soup.select('.listing') or
                 soup.select('article'))
        # Fallback: build cards from listing links
        if not cards or (len(cards) == 1 and not cards[0].get_text(' ', strip=True)):
            candidates = []
            ...
            for c in candidates:
                if id(c) not in seen:
                    seen.add(id(c))
                    cards.append(c)
        print(f"  BinaTap kartoçka: {len(cards)}")
        ...
        for card in cards:
            try:
                ...
                print(f"  ⚠️ {e}")
    print(f"  BinaTap cəmi: {len(results)}")
    return results
