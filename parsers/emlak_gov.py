"""
EvTap — Emlak.gov.az parser
Dövlət daşınmaz əmlak portalı
"""
...
SOURCE = 'emlak_gov'
SOURCE_NAME = 'Emlak.gov.az'
BASE_URL = 'https://emlak.gov.az'


def parse_emlak_gov(pages=2):
    results = []
    for page in range(1, pages + 1):
        ...
        cards = (soup.select('.item') or soup.select('.listing') or
                 soup.select('article') or soup.select('tr'))
        print(f"  Emlak.gov kartoçka: {len(cards)}")
        ...
        for card in cards:
            try:
                ...
                print(f"  ⚠️ {e}")
    print(f"  Emlak.gov cəmi: {len(results)}")
    return results
...
def _parse_card(card):
    ...
    price_el = card.select_one('.price') or card.select_one('[class*="price"]') or card.select_one('td:nth-child(3)')
    price = clean_price(price_el.get_text()) if price_el else None

    title_el = card.select_one('td') or card.select_one('h3') or card.select_one('.title')
