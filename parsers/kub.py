...
SOURCE = 'kub'
SOURCE_NAME = 'Kub.az'
BASE_URL = 'https://kub.az'

URLS = {
    'satis': 'https://kub.az/sale?p={page}',
    'kiraye': 'https://kub.az/rent?p={page}',
}


def parse_kub(pages=2):
    results = []
    for deal_type, url_template in URLS.items():
        for page in range(1, pages + 1):
            ...
            cards = (soup.select('.ad') or soup.select('.item') or
                     soup.select('article') or soup.select('.property'))
            print(f"  Kub [{deal_type}] kartoçka: {len(cards)}")
...
def _parse_card(card, deal_type='satis'):
    ...
    price_el = card.select_one('.price') or card.select_one('.cost') or card.select_one('[class*="price"]')
